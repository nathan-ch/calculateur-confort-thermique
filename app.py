...
with col1:
    tdb = st.number_input("Température de l'air (°C)",
                          value=DEFAULT_TDB, step=1.0)
    tr = st.number_input("Température radiante moyenne (°C)",
                         value=DEFAULT_TR, step=1.0)
...
# Calcul PMV standard
resultats_pmv = pmv_ppd_iso(
    tdb=tdb, tr=tr, vr=vr, rh=rh, met=met, clo=clo_dynamic_iso, limit_inputs=False)
st.session_state.pmv = resultats_pmv['pmv']
st.session_state.ppd = resultats_pmv['ppd']
...
with col3:
    st.metric(label="Température opérative",
              value=f"{st.session_state.operative_tmp:.2f}°C")
...
if st.session_state.pmv != 0 or st.session_state.ppd != 0:
    ax.scatter(st.session_state.pmv, st.session_state.ppd,
               color='red', s=100, label='Résultat actuel')
